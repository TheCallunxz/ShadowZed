...
local version,author,lVersion = "v2.0","TheCallunxz","8.10"
...
function ShadowZed:LoadMenu()
    ...
    ZedMenu.Pred:MenuElement({id = "pred", name = "Pred to use",  drop = {"Noddy", "HPred", "WRPred"}, value = 1})
...
function ShadowZed:GetPredHitChance(heroPos, target, range, speed, delay, width, collision)
    canhit = 1
    hitchance, aimpos = HPred:GetHitchance(heroPos, target, range, speed, delay, width, collision)
...
function ShadowZed:Dodging()
    ...
    if(((myHero:GetSpellData(_W).name == "ZedW2") or HasBuff(myHero, "ZedWHandler") or (Game.CanUseSpell(_W) == 0)) and readyToW == false) then
        readyToW = true
    end
